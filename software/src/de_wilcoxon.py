...
def main(expr_path, meta_path, group1, group2, output, condition_col, 
    ...
    unique_cell_ids = df_expr_pl['UniqueCellId'].cat.get_categories().gather(unique_row_codes_phys).to_pandas()
    unique_gene_ids = df_expr_pl['Ensembl Id'].cat.get_categories().gather(unique_col_codes_phys).to_pandas()

    # Delete Polars objects to free memory
    del df_expr_pl, row_codes_raw, col_codes_raw
